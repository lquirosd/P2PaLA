...
def matching_structure(hyp_poly,target_poly, epsilon=0.1, w=[0.0,0.5,0.5,1.0,1.0,1.0]):
    """
    """
    img = cv2.imread('/home/lquirosd/WORK/PhD/P2PaLA/to_delete/170025120000003,0611.tif')
    ...
    o2o = np.logical_and(np.isclose(sigma, 1, atol=epsilon), np.isclose(tao,1,atol=epsilon)).sum()
    ...
    f = (np.array(w)*np.array([2*o2o,m2o,o2m,o2z,z2o,m2m])).sum()/(g_len+d_len)
    return f
